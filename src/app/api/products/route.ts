...
import { error400, error401, error403, error500, formateDate, success200 } from "@/lib/utils"
import { ZodProductSchema } from "@/lib/zod-schemas/schema"
import { getServerSession } from "next-auth"
import type { NextRequest } from "next/server"
import type { z } from "zod"
import type { ColorVariant } from "@/lib/types/types"
  ...
  try {
    ...
    return success200({
      products: products.map((product) => ({
        ...
        color: product.color,
        image: product.Image.find((image) => image.imagePublicId.endsWith("-thumb"))?.imagePublicId,
